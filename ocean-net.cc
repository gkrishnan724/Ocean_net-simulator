...
Ptr<FileAggregator> dtnAggregator;
...
CourseChange(std::ostream *myos, std::string foo, Ptr<const MobilityModel> mobility)
{
  ...
  *myos << Simulator::Now() << "; NODE: " << node->GetId() << "; POS: x=" << pos.x << ", y=" << pos.y << ", z=" << pos.z << "; VEL: x=" << vel.x << ", y=" << vel.y << ", z=" << vel.z << std::endl;
}
...
void DtnApp::StartApplication(void)
{
  ...
  // // STDMA
  // Ptr < stdma::StdmaNetDevice > dev = DynamicCast < stdma::StdmaNetDevice > (m_node -> GetDevice(0));
  // NS_ASSERT(dev != NULL);
  // PointerValue ptr;
  // dev -> GetAttribute("Mac", ptr);
  // Ptr < stdma::StdmaMac > mac = ptr.Get < stdma::StdmaMac > ();
  // NS_ASSERT(mac != NULL);
  // mac_queue = mac -> GetQueue();
  // NS_ASSERT(mac_queue != NULL);
...
{
  std::cout << "TCP connection fails at time " << Simulator::Now().GetSeconds() << " at node " << m_node->GetId() << "\n";
}
...
void DtnApp::PrintBuffers(void)
{
  ...
  std::cout << Simulator::Now().GetSeconds() << " " << mac_queue->GetSize() << " " << m_antipacket_queue->GetNPackets() << " " << m_queue->GetNPackets() << " " << m_queue->GetNBytes() << " " << currentNeighbors << " " << mobility->GetPosition().x << " " << mobility->GetPosition().y << "\n";
  Simulator::Schedule(Seconds(1.0), &DtnApp::PrintBuffers, this);
}

void DtnApp::CheckBuffers(uint32_t bundletype)
{
  Ptr<Packet> packet;
...
void DtnApp::SendBundle(uint32_t dstnode, uint32_t packetsize)
{
  ...
  if ((m_queue->GetNBytes() + m_antipacket_queue->GetNBytes() + packet->GetSize()) <= b_s)
  {
    ...
    if (success)
    {
      std::cout << "At time " << Simulator::Now().GetSeconds() << " send bundle with sequence number " << bndlHeader.GetOriginSeqno() << " from " << bndlHeader.GetOrigin() << " to " << bndlHeader.GetDst() << "\n";
    }
  }
  else
  {
    std::cout << "At time " << Simulator::Now().GetSeconds() << " tried to send bundle with sequence number " << bndlHeader.GetOriginSeqno() << " from " << bndlHeader.GetOrigin() << " to " << bndlHeader.GetDst() << "\n";
  }
}
...
void DtnApp::SendAP(Ipv4Address srcstring, Ipv4Address dststring, uint32_t seqno, Time srctimestamp)
{
  ...
  bool success = m_antipacket_queue->Enqueue(packet);
  if (success)
    std::cout << "At time " << Simulator::Now().GetSeconds() << " send antipacket with sequence number " << apHeader.GetOriginSeqno() << " original ts " << srctimestamp.GetSeconds() << " new ts " << apHeader.GetSrcTimestamp().GetSeconds() << " from " << apHeader.GetOrigin() << " to " << apHeader.GetDst() << "\n";
}
...
void DtnApp::DstHandleConnectionCreated(Ptr<Socket> s,
                                        const Address &addr)
{
...
void DtnApp::ReceiveBundle(Ptr<Socket> socket)
{
  ...
  while (socket->GetRxAvailable() > 0)
  {
    ...
    else
    {
      ...
      {
        std::cout << "WTF at time " << Simulator::Now().GetSeconds() << " received " << p->GetSize() << " bytes at " << owniaddress.GetIpv4() << " total bytes " << currentServerRxBytes[i] << " from " << address.GetIpv4() << " seqno " << src_seqno << "\n";
      }
      else
      {
        currentServerRxBytes[i] += p->GetSize();
    ...
    if (currentServerRxBytes[i] == bundle_size[i])
    {
      ...
      else
      {
        ...
        if (IsDuplicate(qpkt, m_queue) == 1)
          std::cout << "At time " << Simulator::Now().GetSeconds() << " received duplicate " << newpkt[i]->GetSize() << " bytes at " << owniaddress.GetIpv4() << " from " << address.GetIpv4() << " bundle hop count: " << (unsigned)bndlHeader.GetHopCount() << " sequence number: " << bndlHeader.GetOriginSeqno() << " bundle queue occupancy: " << m_queue->GetNBytes() << "\n";
        if ((IsDuplicate(qpkt, m_queue) == 0) && (AntipacketExists(qpkt) == 0) && ((Simulator::Now().GetSeconds() - bndlHeader.GetSrcTimestamp().GetSeconds()) < 750.0))
        {
          ...
          {
            std::cout << "At time " << Simulator::Now().GetSeconds() << " received " << newpkt[i]->GetSize() << " bytes at " << owniaddress.GetIpv4() << " (final dst) from " << address.GetIpv4() << " delay: " << Simulator::Now().GetSeconds() - bndlHeader.GetSrcTimestamp().GetSeconds() + 1000.0 * (bndlHeader.GetNretx()) << " bundle hop count: " << (unsigned)bndlHeader.GetHopCount() + 1 << " sequence number: " << bndlHeader.GetOriginSeqno() << " bundle queue occupancy: " << m_queue->GetNBytes() << "\n";
          ...
          else
          {
            ...
            {
              drops++;
              std::cout << "At time " << Simulator::Now().GetSeconds() << " dropped " << newpkt[i]->GetSize() << " bytes at " << owniaddress.GetIpv4() << " from " << address.GetIpv4() << " bundle hop count: " << (unsigned)bndlHeader.GetHopCount() << " sequence number: " << bndlHeader.GetOriginSeqno() << " bundle queue occupancy: " << m_queue->GetNBytes() << "\n";
            }
          }
        }
      }
  ...
  if (first == 0)
  {
    ...
    if (now < endTime)
    {
      ...
      for (uint32_t i = 0; i < pkts; ++i)
      {
        ...
        {
          std::cout << "At time " << Simulator::Now().GetSeconds() << " too big Hello (B) (" << msg.str().length() << ") bytes.\n";
        }
        bool success = m_queue->Enqueue(p);
        if (success)
        {
      ...
      for (uint32_t i = 0; i < apkts; ++i)
      {
        ...
        {
          std::cout << "At time " << Simulator::Now().GetSeconds() << " too big Hello (AP) (" << msg.str().length() << ") bytes.\n";
        }
        bool success = m_antipacket_queue->Enqueue(p);
        if (success)
        {
...
public:
  ...
  void ScheduleTx(uint32_t dstnode, Time tNext, uint32_t packetsize);
  void SendPacket(uint32_t dstnode, uint32_t packetsize);
...
void MyApp::SendPacket(uint32_t dstnode, uint32_t packetsize)
{
   Ptr<Packet> packet = Create<Packet>(packetsize);
   char dststring[1024] = "";
   sprintf(dststring, "10.0.0.%d", (dstnode + 1));
  //  std::cout<<dststring<<"\n";
   InetSocketAddress dstlocaladdr(Ipv4Address(dststring), 80);
   
   m_socket->SendTo(packet,0,dstlocaladdr);
}

void MyApp::ScheduleTx(uint32_t dstnode, Time tNext, uint32_t packetsize)
{
  m_sendEvent = Simulator::Schedule(tNext, &MyApp::SendPacket, this, dstnode, packetsize);
}
...
public:
  ...
  Simulation(uint32_t scenario,uint32_t shoreNodes,uint32_t travelNodes,uint32_t clusterNodes,Ptr<FileAggregator> agg) : Simulation()
  {
    m_route_proto = scenario;
    m_nBaseRangeNodes = shoreNodes;
    m_nTravelNodes = travelNodes;
    m_nClusterNodes = clusterNodes;
    aggregator = agg;
  }
  ...
  uint32_t m_nClusterNodes;
  uint32_t m_nTravelNodes;
  uint32_t m_nBaseRangeNodes;
  ...
  NodeContainer m_travellingNodes;
  NodeContainer m_baseStationRangeNodes;
...
void Simulation::ConfigureNodes()
{
  ...
  else
  {
    ...
    m_travellingNodes.Create(m_nTravelNodes);
    m_baseStationRangeNodes.Create(m_nBaseRangeNodes);
    m_clusterNodes.Create(m_nClusterNodes);

    m_allNodes.Add(m_baseNodes);
    m_allNodes.Add(m_travellingNodes);
    m_allNodes.Add(m_baseStationRangeNodes);
    m_allNodes.Add(m_clusterNodes);
  }
}
...
void Simulation::ConfigureChannels()
{
  ...
  else
  {
    ...
    m_adaptiveNodes.Add(m_travellingNodes);
    m_adaptiveNodes.Add(m_baseStationRangeNodes);

    m_baseDevices = wifi.Install(basePhy, wifiMac, m_baseNodes);
    m_adaptiveDevices = wifi.Install(ABEPhy, wifiMac, m_adaptiveNodes);
    m_arDevices = wifi.Install(arNodePhy, wifiMac, m_clusterNodes);

    std::cout << m_arDevices.GetN() << "\n";
    std::cout << m_adaptiveDevices.GetN() << "\n";
    std::cout << m_baseDevices.GetN() << "\n";
...
void Simulation::ConfigureMobility()
{
  ...
  else
  {
    ...
    ssPause << "ns3::UniformRandomVariable[Min=0.0|Max=0]";

    ObjectFactory pos;
    pos.SetTypeId("ns3::RandomBoxPositionAllocator");
    pos.Set("X", StringValue("ns3::UniformRandomVariable[Min=50.0|Max=1000]"));
    pos.Set("Y", StringValue("ns3::UniformRandomVariable[Min=10.0|Max=9500.0]"));
    ...
    mobility.Install(m_baseStationRangeNodes);

    //Define Grids and bounds for each type of node, following this convention.

    // 1. Define clusters, and each cluster will have some nodes that will have a bounds of 400m lengths
    // Random WALK for nodes within the cluster.

    //2. From the range_check, we were able to determine that the base station can transmit upto 25km .
    // WE will define these clusters they will be far from 25km. Each cluster will have a 2km seperation.
    // Traveling nodes, will travel to cluster end points and to the base station with random Way point. So they are free to move anywhere
    //In that vast range.

    //3. m_baseRangeNodes will travel with randomWaypoint within the range of the baseStation they wont cross 25km mark.

    MobilityHelper mobility;
   
    mobility.SetPositionAllocator("ns3::GridPositionAllocator",
                                  "MinX", DoubleValue(50),
                                  "MinY", DoubleValue((15000.0)),
     ...
     mobility.SetMobilityModel ("ns3::RandomWalk2dMobilityModel",
                            "Bounds", RectangleValue (Rectangle (0,400, 15000, 15000+400)),
                            "Speed",StringValue(ssSpeed.str()),
                            "Distance",DoubleValue(10.0));

    mobility.Install(m_clusterNodes);
    ...
    Xval << "ns3::UniformRandomVariable[Min=0.0|Max=" << maxX << "]";
    std::stringstream Yval;
    Yval << "ns3::UniformRandomVariable[Min=50|Max=" << maxY << "]";

    pos2.SetTypeId("ns3::RandomBoxPositionAllocator");
    pos2.Set("X", StringValue(Xval.str()));
    pos2.Set("Y", StringValue(Yval.str()));

    std::stringstream ssSpeed2;
    ssSpeed2 << "ns3::UniformRandomVariable[Min=0.0|Max=15]";
    std::stringstream ssPause2;
    ssPause2 << "ns3::UniformRandomVariable[Min=0.0|Max=0]";
    ...
    mobility.SetPositionAllocator("ns3::GridPositionAllocator",
                                  ...
                                  "DeltaY", DoubleValue(800.0),
                                  "GridWidth", UintegerValue(3),
                                  "LayoutType", StringValue("RowFirst"));

    mobility.SetMobilityModel("ns3::RandomWaypointMobilityModel",
                              "Speed", StringValue(ssSpeed2.str()),
                              "Pause", StringValue(ssPause2.str()),
                              "PositionAllocator", PointerValue(taPositionAlloc2));

    mobility.Install(m_travellingNodes);
  }
}
...
void Simulation::ConfigureApplications()
{
  ...
  else
  {
    ...
    { //DTN
      ...
      for (uint32_t j = 0; j < totalPackets; ++j)
      {
        uint32_t dstnode = x.GetInteger(1, m_allNodes.GetN() - 1);
        app[0]->ScheduleTx(dstnode, Seconds(j + 10), 1024);
      }
    ...
    { //Rest of the stuff.
      ...
      Ptr<Socket> ns3Socket = Socket::CreateSocket(m_baseNodes.Get(0), udp_tid);
      Ptr<Ipv4> ipv4 = m_allNodes.Get(0)->GetObject<Ipv4>();
      Ipv4Address ipaddr = (ipv4->GetAddress(1, 0)).GetLocal();
      InetSocketAddress local = InetSocketAddress(ipaddr, 80);
      ns3Socket->Bind(local);
      ns3::UniformVariable x;
      Ptr<MyApp> app = CreateObject<MyApp>();
      uint32_t dstnode;
      
      app->Setup(ns3Socket, InetSocketAddress(m_allInterfaces.GetAddress(dstnode), 80), 1000000, 200, DataRate("1Mbps"));
      m_baseNodes.Get(0)->AddApplication(app);
      app->SetStartTime(Seconds(1.));
      app->SetStopTime(Seconds(m_totalSimTime));

      for(uint32_t i=0;i<totalPackets;i++){
        dstnode = x.GetInteger(1,m_allNodes.GetN()-1);
        app->ScheduleTx(dstnode,Seconds(i+10),1024);
      }
...
void Simulation::ProcessOutputs()
{
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin(); i != stats.end(); ++i)
  {
    ...
    bool skipCond = (t.destinationAddress == "10.255.255.255") || t.sourceAddress != "10.0.0.1" || i->second.txBytes < 1024;
    if(skipCond)
      continue;
    ...
    avgThroughput += i->second.rxBytes * 8.0 / (i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds()) / 1000;
    totalflows++;
  ...
  std::cout << "Total Packets Lost: " << totalPackets - rxPacketsum << "\n";
  std::cout << "Packet Delivery Ratio: " << ((rxPacketsum * 100) / totalPackets) << "\n";
  std::cout << "Packets Lost Ratio: " << (((totalPackets - rxPacketsum) * 100) / totalPackets) << "%"
            << "\n";
  std::cout << "Avg delay: "<<delay/totalflows<<"\n";
  
  double pdr = ((rxPacketsum * 100) / totalPackets);
  if(pdr > 100){
    pdr = ((rxPacketsum * 100) / txPacketsum);
  }

  if(m_route_proto != 5){
    aggregator->Write6d("Dataset/Context/String",m_nBaseRangeNodes,m_nTravelNodes,m_nClusterNodes,pdr,avgThroughput/totalflows,delay/totalflows);
  }
  Simulator::Destroy();
}

void Simulation::PopulateArpCache()
{
...
int main(int argc, char **argv)
{
  ...
  std::string fileName       = "performance-olsr-eq.csv";
  ...
  // aggregator->Enable ();
  // Ptr<FileAggregator> aggregator3 =
  // CreateObject<FileAggregator> ("performance-dsdv-eq.csv", FileAggregator::COMMA_SEPARATED);
  // aggregator3->Enable ();
  // Ptr<FileAggregator> aggregator2 =
  //   CreateObject<FileAggregator> ("performance-aodv-eq.csv", FileAggregator::COMMA_SEPARATED);
  // aggregator2->Enable ();
  // for(int i=1;i<10;i++){
  //   Simulation(1,i*3,i,i,aggregator).Simulate(argc, argv);
  //   Simulation(2,i*3,i,i,aggregator2).Simulate(argc,argv);
  //   Simulation(3,i*3,i,i,aggregator3).Simulate(argc,argv);
  // }

  // aggregator =  CreateObject<FileAggregator> ("performance-olsr-clust.csv", FileAggregator::COMMA_SEPARATED);
  // aggregator2 = CreateObject<FileAggregator> ("performance-aodv-clust.csv", FileAggregator::COMMA_SEPARATED);
  // aggregator3 = CreateObject<FileAggregator> ("performance-dsdv-clust.csv", FileAggregator::COMMA_SEPARATED);
  // aggregator->Enable();
  // aggregator2->Enable();
  // aggregator3->Enable();
  // for(int i=1;i<10;i++){
  //   Simulation(1,i,i,i*3,aggregator).Simulate(argc, argv);
  //   Simulation(2,i,i,i*3,aggregator2).Simulate(argc, argv);
  //   Simulation(3,i,i,i*3,aggregator3).Simulate(argc, argv);
  // }
  // aggregator =  CreateObject<FileAggregator> ("performance-olsr-travel.csv", FileAggregator::COMMA_SEPARATED);
  // aggregator2 = CreateObject<FileAggregator> ("performance-aodv-travel.csv", FileAggregator::COMMA_SEPARATED);
  // aggregator3 = CreateObject<FileAggregator> ("performance-dsdv-travel.csv", FileAggregator::COMMA_SEPARATED);
  // aggregator->Enable();
  // aggregator2->Enable();
  // aggregator3->Enable();
  // for(int i=1;i<10;i++){
  //   Simulation(1,i,i*3,i,aggregator).Simulate(argc, argv);
  //   Simulation(2,i,i*3,i,aggregator2).Simulate(argc, argv);
  //   Simulation(3,i,i*3,i,aggregator3).Simulate(argc, argv);
  // }

    
  CreateObject<FileAggregator> ("Dtn-perf.csv", FileAggregator::COMMA_SEPARATED);
  dtnAggregator->Enable();

  Simulation(5,10,10,10,dtnAggregator).Simulate(argc,argv);
}
